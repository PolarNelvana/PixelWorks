...
;       the window.
;
;   OffsetX, OffsetY - Output variables. [Optional] Offset, in pixels, of the
...
;   Height, OffsetX, and OffsetY) are updated.  See the *Parameters* section
...
;
;-------------------------------------------------------------------------------
WinGetPosEx(hWindow,ByRef X="",ByRef Y="",ByRef Width="",ByRef Height="",ByRef OffsetX="",ByRef OffsetY="")
    {
    Static Dummy56932075
          ,RECTPlus
          ,S_OK:=0x0
    ...
    ;-- If DWM is not used (older than Vista or DWM is not enabled), we're done
    if (DWMRC<>S_OK)
        Return &RECTPlus
    ...
    NumPut(OffsetX:=(Width-GWR_Width)//2,RECTPlus,16,"Int")
    NumPut(OffsetY:=(Height-GWR_Height)//2,RECTPlus,20,"Int")
    Return &RECTPlus
    }
